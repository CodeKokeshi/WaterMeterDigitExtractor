...
SETTINGS_SORT_ORDER = "ui/sort_order"
...
def _auto_pad_label(text: str) -> str:
    """Left-pad a short label to NUM_SEGMENTS with leading zeros.

    Typing '58' becomes '00058', '1' becomes '00001'.
    Only pads when every character is a digit or X (so partial / invalid
    entries are left alone and caught by normal validation).
    """
    normalized = text.strip().upper()
    if 0 < len(normalized) < NUM_SEGMENTS and all(
        ch.isdigit() or ch == UNREADABLE_LABEL_CHAR for ch in normalized
    ):
        normalized = normalized.rjust(NUM_SEGMENTS, "0")
    return normalized
...
class ManualPerspectiveView(QGraphicsView):
    ...
    def nudge_selection(self, dx: float, dy: float) -> bool:
        ...
        self._move_selection_by(QPointF(dx, dy))
        after = self.get_points()
        if before is None or after is None or np.allclose(before, after, atol=1e-4):
            return False
        self.geometry_changed.emit()
        return True
...
class ExportGuideboxView(QWidget):
    ...
    def nudge_frame(self, dx: float, dy: float) -> bool:
        if self._frame is None:
            return False
        self._frame.translate_x += float(dx)
        self._frame.translate_y += float(dy)
        self._rerender()
        return True
...
class DatasetExporterMainWindow(QMainWindow):
    def __init__(self):
        ...
        self._nudge_shortcuts: list[QShortcut] = []

        self._build_ui()
        self._connect_signals()
        self._install_nudge_shortcuts()
    ...
    def _build_ui(self):
        ...
        self._sort_combo.addItem("Filename (A → Z)", "name")
        self._sort_combo.addItem("Date Modified", "mtime")
        ...
        sidebar_layout.addLayout(sort_row)
        ...
        self._rotation_preset_btns: list[QPushButton] = []
        for _angle in (0, 90, 180, 270):
            _btn = QPushButton(f"{_angle}°")
            _btn.setFixedWidth(48)
            _btn.setEnabled(False)
            self._rotation_preset_btns.append(_btn)
        ...
        top_layout.addWidget(QLabel("Presets"), 2, 0)
        for _col, _btn in enumerate(self._rotation_preset_btns, start=1):
            top_layout.addWidget(_btn, 2, _col)
        ...
        self._single_label_entry.setPlaceholderText("0-9 / X  (e.g. 58 → 00058)")
        ...
        self._batch_label_entry.setPlaceholderText("Empty = reuse previous  |  58 → 00058")
        ...
        for _btn in self._rotation_preset_btns:
            _btn.setEnabled(False)
    ...
    def _connect_signals(self):
        ...
        for _angle, _btn in zip((0, 90, 180, 270), self._rotation_preset_btns):
            _btn.clicked.connect(lambda _checked, a=_angle: self._on_rotation_preset(a))
    ...
    def _install_nudge_shortcuts(self):
        for key_text, dx, dy in [
            ("W", 0.0, -1.0),
            ("A", -1.0, 0.0),
            ("S", 0.0, 1.0),
            ("D", 1.0, 0.0),
        ]:
            shortcut = QShortcut(key_text, self)
            shortcut.setContext(Qt.ShortcutContext.WidgetWithChildrenShortcut)
            shortcut.activated.connect(
                lambda dx=dx, dy=dy: self._handle_nudge_shortcut(dx, dy)
            )
    ...
    def _restore_persisted_ui_state(self):
        saved_sort = str(self._settings.value(SETTINGS_SORT_ORDER, "name", type=str))
    ...
    def _load_folder(self, folder: str):
        ...
        if sort_by == "mtime":
            sort_key = lambda path: (path.stat().st_mtime, path.name.lower())
        else:
            sort_key = lambda path: path.name.lower()
        files = sorted(
            (
        ...
        self.statusBar().showMessage(f"Loaded {len(files)} image(s) — sorted by {'date modified' if sort_by == 'mtime' else 'filename'}.")
        self._file_list.setCurrentRow(0)

    def _on_sort_changed(self):
        self._settings.setValue(SETTINGS_SORT_ORDER, self._sort_combo.currentData())
        if self._current_image_folder:
            self._load_folder(self._current_image_folder)
    ...
    def _on_file_selected(self, row: int):
        ...
        # Preserve zoom/scroll when navigating in batch mode.
        # Only save when an image is already displayed (not on the very first load).
        _preserve_zoom = self._batch_mode and self._manual_view._pixmap_item is not None
        _saved_manual_xf      = self._manual_view.transform()                        if _preserve_zoom else None
        _saved_manual_zoom    = self._manual_view._zoom_factor                        if _preserve_zoom else None
        _saved_manual_hscroll = self._manual_view.horizontalScrollBar().value()       if _preserve_zoom else None
        _saved_manual_vscroll = self._manual_view.verticalScrollBar().value()         if _preserve_zoom else None
        _gb_frame_now         = self._guidebox_view._frame                            if _preserve_zoom else None
        _saved_gb_scale       = _gb_frame_now.scale                                   if _gb_frame_now is not None else None
        _saved_gb_tx          = _gb_frame_now.translate_x                             if _gb_frame_now is not None else None
        _saved_gb_ty          = _gb_frame_now.translate_y                             if _gb_frame_now is not None else None
        ...
        # Restore zoom + scroll after template is applied so the view doesn't snap back to fit-to-view.
        if _preserve_zoom:
            if _saved_manual_xf is not None:
                self._manual_view.setTransform(_saved_manual_xf)
                self._manual_view._zoom_factor = _saved_manual_zoom
                # Restore scroll bars AFTER setTransform so Qt recalculates their range first.
                self._manual_view.horizontalScrollBar().setValue(_saved_manual_hscroll)
                self._manual_view.verticalScrollBar().setValue(_saved_manual_vscroll)
            if _saved_gb_scale is not None and self._guidebox_view._frame is not None:
                self._guidebox_view._frame.scale       = _saved_gb_scale
                self._guidebox_view._frame.translate_x = _saved_gb_tx
                self._guidebox_view._frame.translate_y = _saved_gb_ty
                self._guidebox_view._rerender(emit_signal=False)
    ...
    def _on_rotation_preset(self, angle: int):
        self._rotation_slider.setValue(angle)  # triggers _on_rotation_changed via signal
    ...
    def _focused_widget_blocks_wasd(self) -> bool:
        widget = QApplication.focusWidget()
        if widget is None:
            return False
        return isinstance(widget, (QLineEdit, QComboBox))

    def _handle_nudge_shortcut(self, dx_units: float, dy_units: float):
        if self._focused_widget_blocks_wasd():
            return
        self._nudge_active_geometry(dx_units, dy_units)

    def _nudge_active_geometry(self, dx_units: float, dy_units: float):
        if self._current_image is None:
            return
        modifiers = QApplication.keyboardModifiers()
        step = 10.0 if modifiers & Qt.KeyboardModifier.ShiftModifier else 3.0
        dx = dx_units * step
        dy = dy_units * step
        moved = False
        if self._current_mode() == ExtractionMode.MANUAL_PERSPECTIVE:
            moved = self._manual_view.nudge_selection(dx, dy)
        else:
            moved = self._guidebox_view.nudge_frame(dx, dy)
        if moved:
            self.statusBar().showMessage(
                f"Nudged {'manual quad' if self._current_mode() == ExtractionMode.MANUAL_PERSPECTIVE else 'guidebox'} by ({int(dx)}, {int(dy)})."
            )
    ...
    def _current_label(self) -> str:
        return _auto_pad_label(self._single_label_entry.text())
    ...
    def _on_save_current(self):
        ...
        if not self._validate_label_or_warn(label):
            return
        # Reflect padded form back so the field and per-digit boxes show the full value.
        if label != self._single_label_entry.text().strip().upper():
    ...
    def _on_batch_save_next(self):
        ...
        if not self._capture_batch_template():
            return
        label = _auto_pad_label(self._batch_label_entry.text())
